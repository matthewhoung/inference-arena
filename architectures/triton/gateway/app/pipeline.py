"""Triton inference pipeline orchestrator.

This module orchestrates the detection + classification pipeline via Triton:
1. YOLOv5n detection (via Triton gRPC)
2. Crop extraction (in-process)
3. MobileNetV2 classification (via Triton gRPC)
...
class TritonInferencePipeline:
    """Orchestrates detection + classification via Triton.

    Similar to monolithic InferencePipeline but uses Triton gRPC instead
    of in-process ONNX Runtime sessions.

    Attributes:
        triton_client: TritonInferenceClient for gRPC communication
    ...
    def __init__(
        ...
        """Initialize the Triton inference pipeline.

        Args:
            triton_client: Initialized TritonInferenceClient
            labels_file: Path to ImageNet labels file (1000 lines)
        """
        self.triton_client = triton_client
        self.labels = self._load_labels(labels_file)

        # Load thresholds from experiment.yaml
        ...
        logger.info(
            f"Pipeline initialized with {len(self.labels)} labels, "
    ...
    def predict(
        ...
        2. YOLOv5n detection (preprocess → Triton gRPC → NMS)
        3. For each detection:
           - Extract crop
           - MobileNetV2 classification (preprocess → Triton gRPC)
        ...
        if len(detections_letterbox) > 0:
            detections_orig = yolo_result.scale_boxes_to_original(
                detections_letterbox
            )
        else:
